	...
	:QWidget(parent)
{
	...
	background_label->setPixmap(QPixmap("./Resources/img/newCharacter/bg_bottom.png"));
	...
	for (int i = 0; i < WINDOW_PAGE_COUNT; i++)
	{
		painter.drawImage(QRect(WINDOW_WIDTH * i, 0, WINDOW_WIDTH, WINDOW_HEIGHT),
			QImage(QString("./Resources/img/newCharacter/desktop_%1.jpg").arg(i)));
	}
	...
	for (int i = 0; i < WINDOW_BUTTON_COUNT; i++)
	{
		...
		label->setPixmap(QPixmap(QString("./Resources/img/newCharacter/btn_%1.png").arg(i)));
...
void character_widget::translateLanguage()
{
...
void character_widget::mousePressEvent(QMouseEvent* e)
{
...
void character_widget::mouseReleaseEvent(QMouseEvent* e)
{
	int xpos = 0;
...
void character_widget::mouseMoveEvent(QMouseEvent* e)
{
	int x = 0;
	if (mouse_press)
	{
...
void character_widget::keyPressEvent(QKeyEvent* e)
{
	if (label_move)
	{
		switch (e->key())
...
void character_widget::changeCurrentPage(CLabel* label)
{
	for (int i = 0; i < WINDOW_BUTTON_COUNT; i++)
	{
...
void character_widget::changeCurrentButton()
{
	for (int i = 0; i < WINDOW_BUTTON_COUNT; i++)
	{
		if (i != current_index)
...
inline void character_widget::setLabelMove(bool enable)
{
	label_move = enable;
}

void character_widget::moveCurrentPage(bool direction)
{
	//改变当前页面对应的按钮
	changeCurrentButton();
